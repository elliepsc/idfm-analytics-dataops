...
# Add dags/utils to path for direct import
sys.path.insert(
    0,
    str(Path(__file__).parent.parent.parent
        / "orchestration" / "airflow" / "dags" / "utils")
)
...
def _make_bq_rows(date_counts: dict):
    ...
    for date_str, count in date_counts.items():
        row = Mock()
        row.validation_date = date_str  # str — monitoring.py compares str(row.validation_date)
...
class TestCheckStatisticalAnomalyNormal:
    ...
    def test_result_contains_all_expected_keys(self):
        ...
        expected_keys = {
            "today_count", "mean_7d", "std_7d",
            "z_score", "is_anomaly", "direction", "execution_date"
        }
...
class TestCheckStatisticalAnomalyBQFailure:
    ...
    def test_bq_exception_returns_default_without_raising(self):
        ...
        with patch("google.cloud.bigquery.Client", side_effect=Exception("BQ unavailable")):
            result = check_statistical_anomaly(
                project_id="test-project",
                execution_date="2024-01-07",
            )
