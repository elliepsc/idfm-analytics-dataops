...
class ODSv2Client:
    ...
    def get_all_records_with_filter(
        ...
        Args:
            filter_field: Field to filter on (e.g., 'code_insee')
            filter_values: List of values to filter by
            select: Comma-separated list of fields
            max_records: Maximum records to fetch (None = unlimited)

        Returns:
            List of all records across all filter values
        """
        all_records = []

        for value in filter_values:
            logger.info(f"Fetching records for {filter_field}={value}")
            where_clause = f"{filter_field}='{value}'"
            ...
            all_records.extend(records)
            logger.info(f"Fetched {len(records)} records for {filter_field}={value}")
