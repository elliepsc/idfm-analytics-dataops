...
Usage: python scripts/validate_data_quality.py --dataset validations
"""
import json
import logging
import sys
...
def validate_validations(data_dir: str = "data/raw") -> bool:
    ...
    context = gx.get_context(
        mode="file", project_root_dir="great_expectations"
    )

    # Trouver le fichier le plus récent
    data_path = Path(data_dir)
    files = sorted(data_path.glob("validations_*.json"))
    if not files:
    ...
    # Valider
    suite = context.suites.get("validations_rail_quality")
