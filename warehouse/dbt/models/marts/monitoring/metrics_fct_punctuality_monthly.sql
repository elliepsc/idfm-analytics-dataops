-- Monitoring metrics for fct_punctuality_monthly
{{
  config(
    materialized='table',
    description='Data health metrics for fct_punctuality_monthly (freshness, volume, nulls, duplicates)'
  )
}}

-- FIX V2: file was truncated in V1 — started mid-CTE with no WITH or config block.

WITH punctuality_metrics AS (
  SELECT
